...
function extractData(iframe, should_get_exam_instances) {
	...
	var teacher_name = teachersTable.innerText.split("מרצה:")[1].trim();
	for (var i = 0; i < teachersTable.rows.length; i++) {
		...
		for (var j = 0; j < teachersTable.rows[i].cells.length-1; j++) {
			if (teachersTable.rows[i].cells[j].innerText.indexOf("מרצה") != -1) {
				teacher_name = teachersTable.rows[i].cells[j+1].innerText.trim();
				break;
			}
		}
	}
	...
	var course_code = course_data[0].innerHTML.split("-")[0];
	course_code=addHypenInIndex(course_code,4);
	var year = year_sem[0];
